...
const ApprovedBrands = () => {
  ...
  let carouselData = useApprovedBrands({
    orderBy: "createdAt_DESC",
    value: "",
    first: 100,
    skip: 0,
  ...
  if (loading || cloading || carouselData.loading) {
    return <Loader />;
  }
  ...
  const filteredCarousel =
    ...
    !carouselData.error &&
    carouselData?.data?.approvedBrands.filter((ft) => ft.featured);
  ...
  return (
    <>
      <section className="w-full mb-12 flex flex-col justify-center items-center">
        <div className="w-full px-6 md:px-3 my-4" id="search"></div>
        <div
        ...
        >
          ...
          <div>
            <ApprovedBrandsCarousel data={filteredCarousel} />
          </div>
        </div>
        <div
        ...
        <div className="w-full px-6 md:px-3 my-4" id="search">
          <Search pageName="Alternatives" isBrandSearch={true} onSearch={handleSearch()} />
        </div>
